...
public class DriveablePart
{
	...
	public RidingEntityPosition rayTraceRider(EntityDriveable driveable, CollisionTest tester)
	{
		...
		if(type == EnumDriveablePart.turret && driveable.getSeats()[0] != null)
		{
			//Define box verticies, where z > 0 is right. See shapeboxes in the toolbox for a visual reference
	...
	public float hitByBullet(BulletType bulletType, float damage, DriveableHit hit, float penetratingPower) {
		return hitByBullet(bulletType, damage, hit, penetratingPower, 1F);
	}
	
	/**
	 * Called when the bullet decided that it hit this driveable part
	 */
	public float hitByBullet(BulletType bulletType, float damage, DriveableHit hit, float penetratingPower, float damageMod)
	{
		//Perform damage code
		float damageModifier = damageMod;
		if (penetratingPower <= 0.7F * penRes && FlansMod.useNewPenetrationSystem) {
			damageModifier *= (float)Math.pow((penetratingPower / (0.7F * penRes)), 2.5);
		}

		health -= damage * bulletType.damageVsDriveable * damageModifier;
